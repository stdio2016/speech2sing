...
function analyzePitch(buf, smpRate) {
  ...
  for (i = fftSize; i < buf.length; i += fftSize) {
    ...
    var max = 0, freq = 0;
    for (j = Math.floor(fftSize/smpRate * 80); j < fftSize/smpRate * 500; j++) {
      var smp2 = (amps[j*2] + amps[j*2+1]) / 2;
      var smp3 = (amps[j*3] + amps[j*3+1] + amps[j*3+2]) / 3;
      var smp4 = (amps[j*4] + amps[j*4+1] + amps[j*4+2]) / 3;
      var totle = (amps[j] + smp2 + smp3 + smp4) / Math.sqrt(j);
      if (totle > max) {
        max = totle;
        freq = j;
      }
    }
    ans.push([i/smpRate, freq * smpRate/fftSize]);
  }
  return ans;
}
