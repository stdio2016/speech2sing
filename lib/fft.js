...
stdio2018.FFT.prototype = {
  ...
  */
  ...
  realIFFT: function (din, dout) {
    var n = din.length/2;
    if (!dout) dout = new Float64Array(din.length);
    var a, b, c, d;
    a = din[0];
    b = din[1];
    dout[0] = (a + b) * 0.5;
    ...
    dout = this.transform(dout, dout, true);
    return dout;
  }
};

(function stdio2018FftTest(){
  // only test this in main thread
  if (!('window' in this)) return;
  var repeat = 1;
